...
from google import genai # Orijinal import ifadesine geri dönüldü
from google.genai import types

# Logging yapılandırması
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s') # SyntaxError düzeltildi
...
if not GEMINI_API_KEY:
    logging.error("GEMINI_API_KEY ortam değişkeni ayarlanmamış.")
    client = None
else:
    try:
...
# Vercel gibi sunucusuz ortamlarda uygulama bağlamı (app_context) garanti edilmeyebilir.
# Bu nedenle, yükleme işlemini doğrudan uygulama başlatılırken çağırıyoruz.
load_product_data()
...
def recommend():
    ...
    try:
        ...
        logging.warning(f"Gemini API isteği engellendi: {e}")
